...
def get_poses_along_filament_backbones(
    ...
    filament_polarity_known: bool = typer.Option(
        True,
        help="Whether filament polarity from annotations should be fixed "
        "during refinement.",
    ),
):
    ...
    for file in annotation_files:
        ...
        scale_factor = pixel_size * float(
            global_df.loc[tilt_series_id, "rlnTomoTomogramBinning"]
        )
        tomo_size = (
            global_df.loc[tilt_series_id][
        ...
        for filament_id, df in filament_df.groupby("rlnTomoManifoldIndex"):
            ...
            rotated_eulers = (
                R.from_matrix(rotated_orientations)
                .inv()
                .as_euler(
                    seq="ZYZ",
                    degrees=True,
    ...
    df2 = pd.DataFrame(
        {
            "rlnTomoParticlesFile": [output_file],
