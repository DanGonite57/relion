...
void CtffindRunner::read(int argc, char** argv, int rank) {
  ...
  fn_in =
      parser.getOption("--i",
                       "STAR file with all input micrographs, or a unix "
                       "wildcard to all micrograph files, e.g. \"mics/*.mrc\"");
  do_use_without_doseweighting =
      parser.checkOption("--use_noDW",
                         "Estimate CTFs from rlnMicrographNameNoDW instead of "
                         "rlnMicrographName (only after MotionCor2)");
  fn_out = parser.getOption("--o",
                            "Directory, where all output files will be stored",
                            "CtfEstimate/");
  do_only_join_results = parser.checkOption(
      "--only_make_star",
      "Don't estimate any CTFs, only join all logfile results in a STAR file");
  continue_old =
      parser.checkOption("--only_do_unfinished",
                         "Only estimate CTFs for those micrographs for which "
                         "there is not yet a logfile with Final values.");
  do_at_most = textToInteger(parser.getOption(
      "--do_at_most",
      "Only process up to this number of (unprocessed) micrographs.", "-1"));
  // Use a smaller squared part of the micrograph to estimate CTF (e.g. to avoid
  // film labels...)
  ctf_win =
      textToInteger(parser.getOption("--ctfWin",
                                     "Size (in pixels) of a centered, squared "
                                     "window to use for CTF-estimation",
                                     "-1"));

  int tomo_section = parser.addSection("Tomography-specific parameters");
  localsearch_nominal_defocus_range = textToFloat(parser.getOption(
      "--localsearch_nominal_defocus",
      "If positive, search defoci (+/-) around rlnTomoNominalDefocus (in A)",
      "10000."));
  bfactor_dose = textToFloat(
      parser.getOption("--exp_factor_dose",
                       "If positive, use exponential factor by which to limit "
                       "maxres per unit dose (maxres*=exp(dose/factor))",
                       "100."));

  int mic_section = parser.addSection("Microscopy parameters");
  // First parameter line in CTFFIND
  Cs =
      textToFloat(parser.getOption("--CS", "Spherical Aberration (mm) ", "-1"));
  Voltage = textToFloat(parser.getOption("--HT", "Voltage (kV)", "-1"));
  AmplitudeConstrast =
      textToFloat(parser.getOption("--AmpCnst", "Amplitude constrast", "-1"));
  angpix = textToFloat(parser.getOption(
      "--angpix", "Pixel size in the input micrographs (A)", "-1"));

  int ctffind_section = parser.addSection("CTFFIND parameters");

  // Second parameter line in CTFFIND
  fn_ctffind_exe =
      parser.getOption("--ctffind_exe",
                       "Location of ctffind executable (or through "
                       "RELION_CTFFIND_EXECUTABLE environment variable)",
                       "");
  box_size = textToFloat(
      parser.getOption("--Box", "Size of the boxes to calculate FFTs", "512"));
  resol_min = textToFloat(parser.getOption(
      "--ResMin", "Minimum resolution (in A) to include in calculations",
      "100"));
  resol_max = textToFloat(parser.getOption(
      "--ResMax", "Maximum resolution (in A) to include in calculations", "7"));
  min_defocus = textToFloat(parser.getOption(
      "--dFMin", "Minimum defocus value (in A) to search", "10000"));
  max_defocus = textToFloat(parser.getOption(
      "--dFMax", "Maximum defocus value (in A) to search", "50000"));
  step_defocus = textToFloat(parser.getOption(
      "--FStep", "defocus step size (in A) for search", "250"));
  amount_astigmatism = textToFloat(
      parser.getOption("--dAst", "amount of astigmatism (in A)", "0"));

  int ctffind4_section = parser.addSection("CTFFIND4 parameters");
  is_ctffind4 = parser.checkOption(
      "--is_ctffind4",
      "The provided CTFFIND executable is CTFFIND4 (version 4.1+)");
  use_given_ps =
      parser.checkOption("--use_given_ps", "Use pre-calculated power spectra?");
  ice_min = textToFloat(parser.getOption(
      "--ice_min", "Minimum frequency for ice ring density (in 1/A)", "0.25"));
  ice_max = textToFloat(parser.getOption(
      "--ice_max", "Maximum frequency for ice ring density (in 1/A)", "0.28"));
  do_extra_ice = parser.checkOption(
      "--extra_ice_stats",
      "When using pre-calculated power spectra, one can calculate extra ice "
      "statistics, which may be useful for selection of icy data sets");
  do_movie_thon_rings = parser.checkOption(
      "--do_movie_thon_rings", "Calculate Thon rings from movie frames?");
  avg_movie_frames = textToInteger(parser.getOption(
      "--avg_movie_frames",
      "Average over how many movie frames (try to get 4 e-/A2)", "1"));
  movie_rootname = parser.getOption(
      "--movie_rootname", "Rootname plus extension for movies", "_movie.mrcs");
  do_phaseshift = parser.checkOption(
      "--do_phaseshift",
      "Estimate the phase shift in the images (e.g. from a phase-plate)");
  phase_min = textToFloat(parser.getOption(
      "--phase_min", "Minimum phase shift (in degrees)", "0."));
  phase_max = textToFloat(parser.getOption(
      "--phase_max", "Maximum phase shift (in degrees)", "180."));
  phase_step = textToFloat(parser.getOption(
      "--phase_step", "Step in phase shift (in degrees)", "10."));
  nr_threads = textToInteger(
      parser.getOption("--j", "Number of threads (for CTFIND4 only)", "1"));
  do_fast_search =
      parser.checkOption("--fast_search",
                         "Disable \"Slower, more exhaustive search\" in "
                         "CTFFIND4.1 (faster but less accurate)");
  do_tilt_search = parser.checkOption(
      "--tilt_search", "Run CTFFIND-4.1.15+ tilt search algorithm");
...
void CtffindRunner::usage() { parser.writeUsage(std::cout); }
...
void CtffindRunner::initialise(bool is_leader) {
  ...
  if (fn_ctffind_exe == "") {
    ...
    if (penv != NULL) fn_ctffind_exe = (std::string)penv;
  }

  fn_shell = "/bin/sh";
  char* shell_name;
  shell_name = getenv("RELION_SHELL");
  if (shell_name != NULL) fn_shell = (std::string)shell_name;

  if (use_given_ps && do_movie_thon_rings)
    REPORT_ERROR(
        "ERROR: You cannot enable --use_given_ps and --do_movie_thon_rings "
        "simultaneously");

  if (use_given_ps && ctf_win != -1)
    REPORT_ERROR(
        "ERROR* You cannot enable --use_given_ps and --ctfWin simultaneously.");

  if (use_given_ps) do_use_without_doseweighting = false;

  if (do_extra_ice && !use_given_ps)
    REPORT_ERROR(
        "ERROR: You cannot use --extra_ice_stats without also using "
        "--use_given_ps");
  ...
  if (fn_in.isStarFile()) {
    ...
    // Check if this is a TomographyExperiment starfile, and if so, unpack into
    // one large metadatatable
    ...
    if (MDin.numberOfObjects() > 0 && !MDin.containsLabel(EMDL_MICROGRAPH_NAME))
      REPORT_ERROR(
          "ERROR: There is no rlnMicrographName label in the input micrograph "
          "STAR file.");

    if (do_use_without_doseweighting && MDin.numberOfObjects() > 0 &&
        !MDin.containsLabel(EMDL_MICROGRAPH_NAME_WODOSE))
      REPORT_ERROR(
          "ERROR: You are using --use_noDW, but there is no "
          "rlnMicrographNameNoDW label in the input micrograph STAR file.");

    if (use_given_ps && MDin.numberOfObjects() > 0 &&
        !MDin.containsLabel(EMDL_CTF_POWER_SPECTRUM))
      REPORT_ERROR(
          "ERROR: You are using --use_given_ps, but there is no "
          "rlnCtfPowerSpectrum label in the input micrograph STAR file.");

    if (is_tomo && (localsearch_nominal_defocus_range > 0.) &&
        !MDin.containsLabel(EMDL_TOMO_NOMINAL_DEFOCUS)) {
      if (verb > 0)
        std::cout << "WARNING: you specified --localsearch_nominal_defocus, "
                     "but there is no rlnTomoNomicalDefocus in the STAR file; "
                     "using min and max defocus instead..."
                  << std::endl;
    ...
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MDin) {
      ...
      if (is_tomo) {
        ...
        if (localsearch_nominal_defocus_range > 0.) {
          ...
          nominal_defocus_micrographs.push_back(
              -10000. * nominal_defocus);  // in positive Angstroms instead of
                                           // negative um!
        }
      }
    }
  ...
  // If voltage or pixel size were not in the input STAR file, set them from the
  // command line options
  if (!obsModel.opticsMdt.containsLabel(EMDL_CTF_CS)) {
    if (Cs < 0.) {
      REPORT_ERROR(
          "ERROR: the input STAR file does not contain the spherical "
          "aberration, and it is not given through --CS.");
    }
  ...
  if (!obsModel.opticsMdt.containsLabel(EMDL_CTF_VOLTAGE)) {
    if (Voltage < 0.) {
      REPORT_ERROR(
          "ERROR: the input STAR file does not contain the acceleration "
          "voltage, and it is not given through --HT.");
    }
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(obsModel.opticsMdt) {
  ...
  if (!obsModel.opticsMdt.containsLabel(EMDL_CTF_Q0)) {
    if (AmplitudeConstrast < 0.) {
      REPORT_ERROR(
          "ERROR: the input STAR file does not contain the amplitude contrast, "
          "and it is not given through --AmpCnst.");
    }
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(obsModel.opticsMdt) {
  ...
  EMDLabel mylabel =
      (is_tomo) ? EMDL_TOMO_TILT_SERIES_PIXEL_SIZE : EMDL_MICROGRAPH_PIXEL_SIZE;
  if (!obsModel.opticsMdt.containsLabel(mylabel)) {
    if (angpix < 0.) {
      REPORT_ERROR(
          "ERROR: the input STAR file does not contain the micrograph pixel "
          "size, and it is not given through --angpix.");
    }
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(obsModel.opticsMdt) {
  ...
  for (long int imic = 0; imic < fn_mic_given_all.size(); imic++) {
    ...
    if (continue_old) {
      ...
      RFLOAT defU, defV, defAng, CC, HT, CS, AmpCnst, XMAG, DStep,
          maxres = -1., valscore = -1., phaseshift = 0., icering = 0.,
          icepowfrac = 0., icekurtosis = 0.;
      if (getCtffindResults(
              fn_microot, defU, defV, defAng, CC, HT, CS, AmpCnst, XMAG, DStep,
              maxres, valscore, phaseshift, icering, icepowfrac, icekurtosis,
              false))  // false: dont warn if not found Final values
      {
    ...
    if (do_at_most >= 0 && fn_micrographs.size() >= do_at_most) {
      if (process_this) {
        ...
        if (!warned) {
          warned = true;
          std::cout << "NOTE: processing of some micrographs will be skipped "
                       "as requested by --do_at_most"
                    << std::endl;
        }
      }
      // If this micrograph has already been processed, the result should be
      // included in the output. So ignore_this remains false.
    }
  ...
  if (is_leader && do_at_most >= 0) {
    std::cout << fn_mic_given_all.size()
              << " micrographs were given but we process only ";
    std::cout << do_at_most << " micrographs as specified in --do_at_most."
              << std::endl;
  }

  // Make symbolic links of the input micrographs in the output directory
  // because ctffind writes output files alongside the input micropgraph
  if (is_leader) {
    char temp[180];
    char* cwd = getcwd(temp, 180);
    currdir = std::string(temp);
    // Make sure fn_out ends with a slash
    if (currdir[currdir.length() - 1] != '/') currdir += "/";
    FileName prevdir = "";
  ...
  if (is_ctffind4 && ctf_win > 0 && do_movie_thon_rings)
    REPORT_ERROR(
        "CtffindRunner::initialise ERROR: You cannot use a --ctfWin operation "
        "on movies.");

  if (verb > 0) {
    std::cout << " Using CTFFIND executable in: " << fn_ctffind_exe
              << std::endl;
    std::cout << " to estimate CTF parameters for the following micrographs: "
              << std::endl;
    if (continue_old)
      std::cout << " (skipping all micrographs for which a logfile with Final "
...
void CtffindRunner::run() {
  if (!do_only_join_results) {
    ...
    if (verb > 0) {
      if (is_ctffind4)
        std::cout << " Estimating CTF parameters using Alexis Rohou's and Niko "
                     "Grigorieff's CTFFIND4.1 ..."
                  << std::endl;
      else
        std::cout
    ...
    for (long int imic = 0; imic < fn_micrographs.size(); imic++) {
      ...
      obsModel.opticsMdt.getValue(EMDL_CTF_CS, Cs,
                                  optics_group_micrographs[imic] - 1);
      obsModel.opticsMdt.getValue(EMDL_CTF_VOLTAGE, Voltage,
                                  optics_group_micrographs[imic] - 1);
      obsModel.opticsMdt.getValue(EMDL_CTF_Q0, AmplitudeConstrast,
                                  optics_group_micrographs[imic] - 1);
      EMDLabel mylabel = (is_tomo) ? EMDL_TOMO_TILT_SERIES_PIXEL_SIZE
                                   : EMDL_MICROGRAPH_PIXEL_SIZE;
      obsModel.opticsMdt.getValue(mylabel, angpix,
                                  optics_group_micrographs[imic] - 1);
    ...
    if (verb > 0) progress_bar(fn_micrographs.size());
  }
...
void CtffindRunner::joinCtffindResults() {
  ...
  for (long int imic = 0; imic < fn_micrographs_all.size(); imic++) {
    ...
    RFLOAT maxres = -999., valscore = -999., phaseshift = -999., icering = 0.,
           icepowerfrac = 0., icekurtosis = 0.;
    bool has_this_ctf = getCtffindResults(
        fn_microot, defU, defV, defAng, CC, HT, CS, AmpCnst, XMAG, DStep,
        maxres, valscore, phaseshift, icering, icepowerfrac, icekurtosis);

    if (!has_this_ctf) {
      std::cerr << " WARNING: skipping, since cannot get CTF values for "
                << fn_micrographs_all[imic] << std::endl;
    } else {
      ...
      if (do_use_without_doseweighting)
        MDctf.setValue(EMDL_MICROGRAPH_NAME_WODOSE,
                       fn_micrographs_ctf_all[imic]);
      MDctf.setValue(EMDL_MICROGRAPH_NAME, fn_micrographs_all[imic]);
      MDctf.setValue(EMDL_IMAGE_OPTICS_GROUP,
                     optics_group_micrographs_all[imic]);
      ...
      if (is_tomo) {
        // Store pre-exposure to sort images on, just in case this program
        // messed up the order...
        MDctf.setValue(EMDL_MICROGRAPH_PRE_EXPOSURE,
                       pre_exposure_micrographs[imic]);
      }
    }
  ...
  if (MDctf.isEmpty())
    REPORT_ERROR(
        (std::string)fn_ctffind_exe +
        " failed to estimate CTF parameters for any micrograph, exiting...");
  ...
  if (is_tomo) {
    ...
    if (verb > 0)
      std::cout
          << " Saving a file called " << fn_out
          << "power_spectra_fits.star for visualisation of Thon ring fits..."
          << std::endl;
    ...
    for (long int t = 0; t < tomogramSet.tomogramTables.size(); t++) {
      FOR_ALL_OBJECTS_IN_METADATA_TABLE(tomogramSet.tomogramTables[t]) {
        MDpower.addObject(
            tomogramSet.tomogramTables[t].getObject(current_object));
      }
    }
  ...
  if (verb > 0) {
    ...
    if (is_tomo)
      std::cout << " Done! Written out: " << fn_out << "tilt_series_ctf.star"
                << std::endl;
    else
      std::cout << " Done! Written out: " << fn_out << "micrographs_ctf.star"
                << std::endl;
  }
  ...
  for (int i = 0; i < plot_labels.size(); i++) {
    ...
    if (MDctf.containsLabel(label)) {
      ...
      CPlot2D* plot2Db =
          new CPlot2D(EMDL::label2Str(label) + " for all micrographs");
      ...
      if (MDctf.numberOfObjects() > 3) {
        ...
        if (label == EMDL_CTF_ICERINGKURTOSIS)
          MDctf.columnHistogram(label, histX, histY, 0, plot2D, -1, -1, 6);
        else
...
void CtffindRunner::getMySearchParameters(long int imic, RFLOAT& my_def_min,
                                          RFLOAT& my_def_max,
                                          RFLOAT& my_maxres) {
  my_maxres = resol_max;
  ...
  if (localsearch_nominal_defocus_range > 0.) {
    ...
    my_def_min = XMIPP_MAX(min_defocus, nominal_defocus_micrographs[imic] -
                                            localsearch_nominal_defocus_range);
    my_def_max =
        nominal_defocus_micrographs[imic] + localsearch_nominal_defocus_range;
  }
}

void CtffindRunner::executeCtffind3(long int imic) {
  FileName fn_mic =
      getOutputFileWithNewUniqueDate(fn_micrographs_ctf[imic], fn_out);
  ...
  if (!fh)
    REPORT_ERROR(
        (std::string) "CtffindRunner::execute_ctffind cannot create file: " +
        fn_script);

  // If given, then put a square window of ctf_win on the micrograph for CTF
  // estimation
  if (ctf_win > 0) {
    ...
    I.read(fn_mic);
    I().setXmippOrigin();
    I().window(FIRST_XMIPP_INDEX(ctf_win), FIRST_XMIPP_INDEX(ctf_win),
               LAST_XMIPP_INDEX(ctf_win), LAST_XMIPP_INDEX(ctf_win));
  ...
  std::string ctffind4_options =
      (is_ctffind4) ? " --omp-num-threads " + integerToString(nr_threads) +
                          " --old-school-input-ctffind4 "
                    : "";

  // Write script to run ctffind
  fh << "#!/usr/bin/env " << fn_shell << std::endl;
  fh << fn_ctffind_exe << ctffind4_options << " > " << fn_log << " << EOF"
     << std::endl;
  // line 1: input image
  if (do_movie_thon_rings)
  ...
  fh << Cs << ", " << Voltage << ", " << AmplitudeConstrast << ", 10000, "
     << angpix << std::endl;
  // line 4: Box, ResMin[A], ResMax[A], dFMin[A], dFMax[A], FStep[A], dAst[A]
  fh << box_size << ", " << resol_min << ", " << my_maxres << ", "
     << my_min_defocus << ", " << my_max_defocus << ", " << step_defocus << ", "
     << amount_astigmatism << std::endl;
  if (is_ctffind4) {
    // line 4: Movie Thon rings:
    // $input_is_stack_of_frames,$number_of_frames_to_average
    if (do_movie_thon_rings)
      fh << " 1  " << integerToString(avg_movie_frames) << std::endl;
    else
      fh << " 0  1" << std::endl;
    // line 5: Phase-shifts: $find_phase_shift,$min_ps,$max_ps,$step_ps (in
    // rads)
    if (do_phaseshift)
      fh << " 1, " << DEG2RAD(phase_min) << ", " << DEG2RAD(phase_max) << ", "
         << DEG2RAD(phase_step) << std::endl;
    else
  ...
  if (system(command.c_str()))
    std::cerr << "WARNING: there was an error in executing: " << command
              << std::endl;
  ...
  if (ctf_win > 0) {
    if (remove(fn_mic_win.c_str()) != 0)
      std::cerr
          << "WARNING: there was an error deleting windowed micrograph file "
          << fn_mic_win << std::endl;
  }
}
...
void CtffindRunner::executeCtffind4(long int imic) {
  ...
  // std::string ctffind4_options = " --omp-num-threads " +
  // integerToString(nr_threads);
  std::string ctffind4_options = "";
  if (use_given_ps) ctffind4_options += " --amplitude-spectrum-input";

  // Write script to run ctffind
  fh << "#!/usr/bin/env " << fn_shell << std::endl;
  fh << "env LC_ALL=C " << fn_ctffind_exe << ctffind4_options << " > " << fn_log
     << " << EOF" << std::endl;
  // line 1: input image
  if (do_movie_thon_rings) {
    fh << fn_mic_win.withoutExtension() + movie_rootname << std::endl;
  ...
  // So, we say "yes" regardless of the version unless "--fast_search" is
  // specified.
  if (!do_fast_search)
    fh << "yes" << std::endl;
  else
...
bool CtffindRunner::getCtffindResults(
    ...
    RFLOAT& maxres, RFLOAT& valscore, RFLOAT& phaseshift, RFLOAT& icering,
    RFLOAT& icepowerfrac, RFLOAT& icekurtosis, bool do_warn) {
  if (is_ctffind4) {
    return getCtffind4Results(fn_microot, defU, defV, defAng, CC, HT, CS,
                              AmpCnst, XMAG, DStep, maxres, phaseshift, icering,
                              icepowerfrac, icekurtosis, do_warn);
  } else {
    icering = icepowerfrac = icekurtosis = 0.;
    return getCtffind3Results(fn_microot, defU, defV, defAng, CC, HT, CS,
                              AmpCnst, XMAG, DStep, maxres, phaseshift,
                              valscore, do_warn);
  }
}

bool CtffindRunner::getCtffind3Results(FileName fn_microot, RFLOAT& defU,
                                       RFLOAT& defV, RFLOAT& defAng, RFLOAT& CC,
                                       RFLOAT& HT, RFLOAT& CS, RFLOAT& AmpCnst,
                                       RFLOAT& XMAG, RFLOAT& DStep,
                                       RFLOAT& maxres, RFLOAT& phaseshift,
                                       RFLOAT& valscore, bool do_warn) {
  FileName fn_root = getOutputFileWithNewUniqueDate(fn_microot, fn_out);
  FileName fn_log = fn_root + "_ctffind3.log";
  ...
  // The loop statement may be necessary for data blocks that have a list AND a
  // table inside them
  ...
  while (getline(in, line, '\n')) {
    ...
    if (line.find("CS[mm], HT[kV], AmpCnst, XMAG, DStep[um]") !=
            std::string::npos ||
        line.find("CS[mm], HT[kV], ac, XMAG, DStep[um]") !=
            std::string::npos)  // GCTF 1.18 B1 changed the line...
    {
      getline(in, line, '\n');
      tokenize(line, words);
      if (words.size() == 5) {
  ...
  if (!Cs_is_found) {
    if (do_warn)
      std::cerr
          << "WARNING: cannot find line with Cs[mm], HT[kV], etc values in "
          << fn_log << std::endl;
    return false;
  }
  if (!Final_is_found) {
    if (do_warn)
      std::cerr << "WARNING: cannot find line with Final values in " << fn_log
                << std::endl;
    return false;
  }
...
bool CtffindRunner::getCtffind4Results(FileName fn_microot, RFLOAT& defU,
                                       RFLOAT& defV, RFLOAT& defAng, RFLOAT& CC,
                                       RFLOAT& HT, RFLOAT& CS, RFLOAT& AmpCnst,
                                       RFLOAT& XMAG, RFLOAT& DStep,
                                       RFLOAT& maxres, RFLOAT& phaseshift,
                                       RFLOAT& icering, RFLOAT& icepowerfrac,
                                       RFLOAT& icekurtosis, bool do_warn) {
  FileName fn_root = getOutputFileWithNewUniqueDate(fn_microot, fn_out);
  FileName fn_log = fn_root + "_ctffind4.log";
  std::ifstream in(fn_log.data(), std::ios_base::in);
  if (in.fail()) return false;
  ...
  while (getline(in2, line, '\n')) {
    ...
    if (line.find("acceleration voltage:") != std::string::npos) {
      ...
      if (words.size() < 19)
        REPORT_ERROR(
            "ERROR: Unexpected number of words on data line with acceleration "
            "voltage in " +
            fn_log);
      ...
      if (words.size() < 7)
        REPORT_ERROR(
            "ERROR: Unexpected number of words on data line below Columns line "
            "in " +
            fn_log);
      Final_is_found = true;
      ...
      if (do_phaseshift) phaseshift = RAD2DEG(textToFloat(words[4]));
  ...
  if (!Cs_is_found) {
    if (do_warn)
      std::cerr
          << " WARNING: cannot find line with acceleration voltage etc in "
          << fn_log << std::endl;
    return false;
  }
  if (!Final_is_found) {
  ...
  if (!av.fail()) {
    std::string s1, s2;
    // skip 5 lines
    for (int i = 0; i < 5; ++i) std::getline(av, s1);
  ...
  // Be careful, because avgrot files from CTFFIND-4.1 are somehow normalized
  // badly, so re-read power spectrum image This only works if given the power
  // spectrum from RELION's motioncorr. Otherwise, fall back on Rafa's approach
  // below
  if (use_given_ps && do_extra_ice) {
    // Use standard deviation in the diagnostics image for ice ring density
    // instead of Rafa's. Calculate stddev of power, relative to stddev of power
    // in a band with lower resolution than ice ring Richard suggested using a
    // higher band as well, but some strange ice artefacts extend well into the
    // higher band...
    Image<RFLOAT> Ips;
    ...
    RFLOAT n = 0., mean = 0., pow_ice = 0., pow_band = 0., n_band = 0., M2 = 0.,
           M3 = 0., M4 = 0.;
    for (long int i = 0; i <= FINISHINGY(Ips());
         i++)  // Use only bottom half of the diagnostic image, as top contains
               // model and averaged spectra!!
      for (long int j = STARTINGX(Ips()); j <= FINISHINGX(Ips()); j++) {
        RFLOAT r2 = RFLOAT(i * i + j * j);
        if (r2 > r2ice_min && r2 < r2ice_max) {
          n += 1.;
          mean += A2D_ELEM(Ips(), i, j);
        } else if (r2 > r2low_min && r2 < r2low_max ||
                   r2 > r2high_min && r2 < r2high_max) {
          n_band += 1.;
    ...
    pow_band /= n_band;
    if (pow_band > 0.)
      icepowerfrac = (mean - pow_band) / pow_band;
    else
      icepowerfrac = 0.;
    ...
    RFLOAT s2 = M2 / (n - 1);
    RFLOAT s = sqrt(s2);
    // RFLOAT skewness = (s > 0.) ? (n / ((n - 1) * (n - 2))) * (M3 / (s * s *
    // s)) : 0.;
    icekurtosis =
        (n * (n + 1) * M4) / ((n - 1) * (n - 2) * (n - 3) * (s2 * s2)) -
        (3 * (n - 1) * (n - 1)) / ((n - 2) * (n - 3));
